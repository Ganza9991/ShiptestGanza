 ...
 * Гордость / Amber
 * Абилки:
	• Случайная область подвергается обстрелу. Сначала выбирается область, потом берутся углы и вычетаются adjustment_turf, чтобы было подобие круга. Через время стрелы градом осыпают область.
	• Из армии (стенок) выходят мобы-легионеры, количество которых равно сражающимся+1.
	• Призывает баннер на 1 из углов, радиус = 50% arena_range. 3 типа банеров на выбор: Деморализующий - дебафает врагов; Воодушевляющий - хилит и даёт лёгкий резист урона; Осадный - снижает кд способок.
	• (Amber enhance only) :
 * Лут:
	• Щит легионера
	• Осколок копья легионера (Кинжал)
...
/datum/tumor_elite_set/legionnaire
	elite = /mob/living/simple_animal/hostile/asteroid/elite/legionnaire
...
/obj/structure/legionnaire_bonfire/expose_reagents(list/reagents, datum/reagents/source, method, volume_modifier, show_message)
	. = ..()
	if(/datum/reagent/water in reagents)
		qdel(src)
...
/obj/structure/legionnaire_bonfire/Initialize()
	...
	smoke.set_up(1, loc)
	smoke.start()
	addtimer(CALLBACK(src, PROC_REF(conjure_broodlings)), 10 SECONDS, TIMER_LOOP)
