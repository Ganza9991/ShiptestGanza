// Эта дрель по большей части имеет тот же код, но мы форсим удаление оригинальной версии и заменяем своей, чтобы не коментить оригинальный код
...
/datum/mission/outpost/drill
	weight = 3 // Снижаем вес миссии ибо легко
...
/obj/machinery/heavy_mining_drill/ui_data(mob/user)
	var/list/data = list()
	data["active"] = active
	data["status"] = get_status_text()
	data["malfunction"] = malfunction
	data["metal_attached"] = metal_attached
	data["broken"] = (machine_stat & BROKEN)
	data["message_log"] = message_log
	return data

/obj/machinery/heavy_mining_drill/proc/get_status_text()
	if(machine_stat & BROKEN)
		return "Structural failure - Requires repairs"
	if(malfunction)
		switch(malfunction)
			if(MALF_LASER)
				return "Laser array malfunction"
			if(MALF_SENSOR)
				return "Sensor calibration error"
			if(MALF_CAPACITOR)
				return "Capacitor failure"
			if(MALF_STRUCTURAL)
				return "Structural integrity compromised"
			if(MALF_CALIBRATE)
				return "Gimbal misalignment"
	if(active)
		return "Mining operation in progress"
	if(our_vein)
		return "Ready for activation"
	return "Idle - No vein detected"
...
/obj/machinery/heavy_mining_drill/proc/deactivate_usr(mob/user)
	if(!active)
		return
...
/obj/machinery/heavy_mining_drill/proc/to_varlog(message)
	if(message)
		message_log += "[station_time_timestamp()]: [message]"
...
/obj/machinery/heavy_mining_drill/interact(mob/user, special_state)
	. = ..()
	if(malfunction)
	...
	if(!our_vein)
		...
		to_varlog("[src] isn't secured over an ore vein!")
		return
	if(!active)
...
/obj/machinery/heavy_mining_drill/proc/mine()
